...
0.8.9: BPIC2012; глобальная сводка мутаций Information по всем directed рёбрам (не только top-15).

Рецепт: `agent_rules`. Конфиг: `configs/diagnostic/v0.8.9.json`.
...
Кандидатов полей: 2.  
Рёбер с мутацией: 0 / 1694 (доля 0.0000).  
Handover с мутацией: 0 / 35448 (доля 0.0000).  
Взвешенное среднее числа полей: 0.0000.  
Глобальный топ полей: —.

Топ рёбер по mutation_mass (avg_n × n):
— (на ненулевых рёбрах Information не меняется)

По tertile handover_count:
- `low` (n=1…4): рёбер 0/564, доля=0.0000
- `mid` (n=4…13): рёбер 0/565, доля=0.0000
- `high` (n=13…1214): рёбер 0/565, доля=0.0000
